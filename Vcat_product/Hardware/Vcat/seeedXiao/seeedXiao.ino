...
void SPIinit(uint8_t tc = TAB_COLOUR)
...
void mqttIntervalPost()
{
    ...
    sprintf(param, "{\"battery\":%f}",battery); //换成对应的标识符
    sprintf(jsonBuf, ALINK_BODY_FORMAT, param);
    Serial.println(jsonBuf);
    boolean d = client.publish(ALINK_TOPIC_PROP_POST, jsonBuf);
    
    if(d){
      Serial.println("publish Battery Voltage success"); 
    }else{
      Serial.println("publish Battery Voltage fail"); 
    }
